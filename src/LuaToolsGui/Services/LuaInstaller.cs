...
/// <summary>Thrown when a lua is refused because its contents don't belong in a Steam manifest. Distinct
/// from an IO failure so callers can tell "we blocked this" from "the disk was busy".</summary>
public sealed class LuaValidationException(string message) : Exception(message);
...
/// NOT (it is Steam's own, a sibling of steamapps). See <c>SteamService.DepotCacheDir</c> — writing
/// manifests under config was a real bug that silently broke every pinned download.
///
...
public partial class LuaInstaller(SteamService steam, SettingsService settings, CacheService cache, LuaVault vault)
{
    ...
    {
        // Screen BEFORE writing, and before any rewrite, so what's judged is what the source actually said.
        // This is the single chokepoint every install path funnels through (InstallLua and InstallZip), so
        // one check here covers the plugin, drag-and-drop, the Add page and Fixes.
        string lua = File.ReadAllText(sourceLuaPath);
        var screen = LuaManifestValidator.Screen(lua);
        if (screen.Rejected)
    ...
    public InstallResult InstallZip(string zipPath, long appId, bool forceLocked = false, string? source = null)
    {
        ...
        string? rejected = null; // first content-screening refusal, if any
        ...
        using (archive)
        {
            ...
            foreach (var entry in archive.Entries)
            {
                ...
                catch (LuaValidationException ex)
                {
                    // A refusal, not an accident — keep the reason so the UI can say why instead of
                    // reporting a generic "couldn't write this file".
                    rejected ??= ex.Message;
                    failed.Add(name);
                }
                catch
        ...
        return new InstallResult(luaInstalled, manifestCount, failed, Error: rejected);
    }
}
